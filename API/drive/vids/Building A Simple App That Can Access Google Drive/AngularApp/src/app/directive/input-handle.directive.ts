...
import {Subject,ReplaySubject,BehaviorSubject, fromEvent,Subscription,combineLatest}  from "rxjs"
import {first} from  "rxjs/operators"
import {RyberService} from '../ryber.service'
import {  objectCopy,eventDispatcher } from '../customExports';
...
export class InputHandleDirective {
    ...
    ngOnInit(){
        ...
        if(environment.inputHandle.lifecycleHooks) console.log('inputHandle fires on ngOnInit')
                    ...
                    ?.forEach((x:any,i)=>{
                        ...
                        .subscribe(()=>{
                            ...
                            if(
                                ...
                                if(environment.inputHandle.linkInit) console.log('aux -> aux')
                            ...
                            // reg -> aux
                                ...
                                // console.log(x)
                                if(environment.inputHandle.linkInit) console.log('reg -> aux')
                                try{
                                    Object.entries(
                            ...
                            // aux -> reg
                                ...
                                if(environment.inputHandle.linkInit) console.log('aux -> reg')
                                //find its aux input
                                let classNameArray = this.el.nativeElement.className.split(" ")
                            ...
                            //reg -> reg
                                ...
                                if(environment.inputHandle.linkInit) console.log('reg -> reg')
                                this.iHLinkActionSub = fromEvent( document.getElementsByClassName(x.target)[0],"jumidaInput")
                                .subscribe(this.linkMapper(x))                          
                                eventDispatcher({
                                    event:"jumidaInput",
    ...
    private linkMapper(x: any): (value: Event) => void {
        return (e) => {
            ...
            if(environment.inputHandle.link) console.log(targetValue,x,e,this.el.nativeElement.className)
    ...
    ngOnDestroy(){   
        if(this.extras?.confirm === 'true'){     // here we seem to map corretly but its not so for dropdown investigate
            let formData = this.co.metadata.formData   
            this.ryber[this.extras.co.valueOf()].metadata.zChildrenSubject.unsubscribe()
            delete formData[this.extras.name.valueOf()]
            this.iHOptionsSub?.unsubscribe?.()
            this.iHOptionsLinkSub?.unsubscribe?.() 
            this.iHOptionsLinkActionSub?.unsubscribe?.()
            this.iHLinkActionSub?.unsubscribe?.()
            this.iHLinkInitSub?.unsubscribe?.()
            this.iHFileSub?.unsubscribe?.()
            this.iHAuxInputSub?.unsubscribe?.()
        }
    }
