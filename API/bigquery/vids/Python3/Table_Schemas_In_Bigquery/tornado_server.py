...
from table_schemas import my_bigquery_client
import time
import datetime
import os
...
class ModuleFinder(importlib.machinery.PathFinder):
    ...
    def __init__(self):
        self.path_map = {"table_schemas":table_schemas.__spec__.loader}
...
def start_app(*args, **kwargs):
    ...
    while loading_error:
        try:
            reload(table_schemas)
            my_bigquery_client_code = table_schemas.my_bigquery_client().execute.__code__
            my_client.env = table_schemas.my_bigquery_client().env
            loading_error = False
