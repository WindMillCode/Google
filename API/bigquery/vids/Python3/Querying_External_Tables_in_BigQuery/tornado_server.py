...
from external_query import my_bigquery_client
import time
import datetime
import os
...
class ModuleFinder(importlib.machinery.PathFinder):
    ...
    def __init__(self):
        self.path_map = {"external_query":external_query.__spec__.loader}
...
def start_app(*args, **kwargs):
    ...
    while loading_error:
        try:
            reload(external_query)
            my_bigquery_client_code = external_query.my_bigquery_client().execute.__code__
            my_client.env = external_query.my_bigquery_client().env
            loading_error = False
